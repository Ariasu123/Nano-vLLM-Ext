...
#
# 【先理解 KV Cache】
# Transformer 生成第 101 个 token 时，仍然需要关注前 100 个 token。
# 如果每次都重新计算前 100 个 token 的 Key 和 Value，会浪费大量计算。
# KV Cache 把每一层已经计算过的 Key/Value 保存下来，Decode 时直接读取。
#
# 【为什么分成 Block】
# 不同请求长度不同、结束时间也不同。如果给每条请求预留一整段最大长度显存，会浪费很多空间。
# 本项目像操作系统管理内存页一样，把 KV Cache 切成固定大小的物理块，需要时再分配。
#
# 【逻辑块与物理块】
